  ...
  return (
    <main className="atlas-shell">
      ...
      <div className="atlas-grid">
        ...
        <CelestialViewer object={object} autoRotate={autoRotate} onAutoRotate={setAutoRotate} comparing={compareOpen} onCompare={() => setCompareOpen(!compareOpen)} orbiting={orbiting} onOrbiting={setOrbiting} onResetState={() => setCompareOpen(false)} />
        ...
        <aside className="info-panel" ref={contentRef} aria-live="polite">
          ...
          <div className="info-title" data-reveal>
            <div><h1>{object.name}</h1><em>{object.subtitle}</em></div><ObjectGlyph object={object} />
          </div>
          ...
          <div className="action-grid" data-reveal>
            <button className={orbiting ? "active" : ""} onClick={() => setOrbiting(!orbiting)}><Play size={15} /> Animate orbit</button>
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section ref={dialogRef} tabIndex={-1} className="learning-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <em>{type === "quiz" ? "Knowledge check" : type === "missions" ? "Exploration log" : "Guided observation"}</em>
        <h2 id="modal-title">{type === "quiz" ? `${object.name} quick quiz` : type === "missions" ? "Missions beyond Earth" : `A closer look at ${object.name}`}</h2>
        ...
        : <div className="lesson-content"><p>{object.description}</p><div className="lesson-hero"><ObjectGlyph object={object} /><span><small>Composition</small><b>{object.facts.composition}</b></span></div><h3>What scientists study</h3><p>{object.significance}</p><ul>{object.hotspots.map((hotspot) => <li key={hotspot.id}><b>{hotspot.label}</b><span>{hotspot.detail}</span></li>)}</ul></div>}
      </section>
    </div>
  );
}
